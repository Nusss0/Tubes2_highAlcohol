...
export function InputPanel({
  ...
  return (
    <section className="relative overflow-hidden rounded-[var(--radius-panel)] border-[3px] border-[var(--color-beige)] bg-[var(--color-navy)]/95 shadow-[10px_10px_0_#121212]">
      ...
      <div className="relative grid gap-5 px-6 py-6 sm:px-8 lg:grid-cols-[0.9fr_1.1fr] lg:gap-6">
        <div className="space-y-5">
          <div className="rounded-[var(--radius-card)] border-[2px] border-[var(--color-beige)] bg-[var(--color-black)] p-4">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 rounded-[var(--radius-card)] border-[2px] border-[var(--color-beige)] bg-[var(--color-eggplant)]/35 p-2">
              {(["html", "url"] as const).map((kind) => (
                <button
                  key={kind}
                  type="button"
                  onClick={() => onChange({ sourceType: kind })}
                  className={`rounded-[var(--radius-control)] px-4 py-3 text-sm font-medium transition ${
          ...
          <label className="block space-y-2 rounded-[var(--radius-card)] border-[2px] border-[var(--color-beige)] bg-[var(--color-black)] p-4">
            ...
            <input
              ...
              className="w-full rounded-[var(--radius-control)] border-[2px] border-[var(--color-beige)] bg-[var(--color-navy)]/45 px-4 py-3 text-sm font-semibold text-[var(--color-white)] outline-none transition placeholder:text-[var(--color-gray)] focus:bg-[var(--color-black)]"
            />
          </label>
          ...
          <label className="block space-y-2 rounded-[var(--radius-card)] border-[2px] border-[var(--color-beige)] bg-[var(--color-black)] p-4">
            ...
            <select
              ...
              className="w-full rounded-[var(--radius-control)] border-[2px] border-[var(--color-beige)] bg-[var(--color-navy)]/45 px-4 py-3 text-sm font-semibold text-[var(--color-white)] outline-none transition focus:bg-[var(--color-black)]"
            >
        ...
        <div className="space-y-5">
          <label className="block space-y-2 rounded-[var(--radius-card)] border-[2px] border-[var(--color-beige)] bg-[var(--color-black)] p-4">
            ...
            {input.sourceType === "url" ? (
              ...
              <textarea
                ...
                value={input.source}
                onChange={(event) => onChange({ source: event.target.value })}
                className="min-h-[20rem] w-full rounded-[var(--radius-card)] border-[2px] border-[var(--color-beige)] bg-[var(--color-navy)]/45 px-4 py-3 text-sm leading-6 text-[var(--color-white)] outline-none transition placeholder:text-[var(--color-gray)] focus:bg-[var(--color-black)]"
          ...
          {error ? (
            <div className="rounded-[var(--radius-card)] border-[2px] border-[var(--color-burgundy)] bg-[var(--color-burgundy)] px-4 py-3 text-sm font-semibold text-[var(--color-white)]">
              {error}
            </div>
          ) : null}
        </div>
      ...
      <div className="border-t-[3px] border-[var(--color-beige)] px-6 py-5 sm:px-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
          <button
            ...
            onClick={onAnalyze}
            disabled={isLoading}
            className="rounded-[var(--radius-card)] border-[2px] border-[var(--color-beige)] bg-[var(--color-light-blue)] px-6 py-3 text-sm font-black uppercase tracking-[0.18em] text-[var(--color-black)] transition hover:-translate-y-0.5 hover:bg-[var(--color-sage)] disabled:cursor-not-allowed disabled:opacity-60"
          >
